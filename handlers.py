...
def greet_user(update,context):
    ...
    update.message.reply_text(
        f"Hello, fellow kids", reply_markup=main_keyboard())
...
def send_random_frog(update, context):
    ...
    context.bot.send_photo(chat_id=chat_id, photo=open(frog_pic_name, 'rb'))
...
def subscribe(update, context):
    ...
    update.message.reply_text("Subscription complete")

def unsubscribe(update, context):
    user = get_or_create_user(db, update.effective_user, update.message.chat.id)
    unsubscribe_user(db, user)
    update.message.reply_text("Unsubscription complete")
